...
`feature/e2e-vault-sharing`. Increment plan for closing the gap D7 left open: a shared
member has no way to decrypt a vault today.
...
- [ ] **Increment 1 — Server: expose public keys.** New `GET /api/users/public-key?email=`
...
- [ ] **Increment 2 — Crypto primitives: RSA-OAEP wrap/unwrap.** `interop.js`:
...
- [ ] **Increment 3 — Wire sharing end to end.** `VaultMemberAddRequest.VaultKeyWrap`;
...
- [ ] **Increment 4 — Attachments to client-side encryption.** `AttachmentClient`
...
- [ ] **Increment 5 — Password Health to client-side.** Delete
...
- [ ] **Increment 6 — Docs + cleanup.** `docs/SPRINTS.md` checked off, "historial y adjuntos"
